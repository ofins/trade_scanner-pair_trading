...
class PairsScanner: 
    ...
    def find_cointegrated_pairs(self, data, sector_name):
        ...
        for ticker1, ticker2 in pairs_to_test:
            try:
                ...
                # Use the direction with the lower p-value
                    ...
                    spread_stats = self.calculate_spread_stats(pair_data[stock_x], pair_data[stock_y])
                    
                    spread = pair_data[stock_y] - spread_stats['hedge_ratio'] * pair_data[stock_x]
                    spread_adf_pvalue = self.stationarity(spread)
